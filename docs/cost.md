...
- Every value is sized as a DynamoDB **String (S)**: its UTF-8 byte length. The
  ...
  typed attributes.
- The 100-byte per-item overhead from the AWS docs is *storage* overhead, not
...
- 1 RCU per 4 KB, rounded up, minimum 1 *unit*.
...
- For a **Query/Scan**, pass the **cumulative** size of every item read —
  DynamoDB rounds the *total* once, not per item. This is why a filter that drops
...
| `update`  | the item stays put, but a *projected* attribute changed                 | 1×     |
...
the index actually *projects* changed — a `KEYS_ONLY` GSI ignores changes to
...
Break a rule and the query returns a naming error (e.g. *"only the last sort key
(date) can use a range; status must be `=`"*) rather than misbehaving — the same
